...
export function flowNodePreview(type: string, data: FlowNodeData) {
  ...
  if (type === "menu") return data.message || "Lista os setores cadastrados";
...
export function FlowBlockNode({ id, data, type, selected }: NodeProps<Node<FlowNodeData>>) {
  ...
  return (
    <div
      className={cn(
        "w-[220px] overflow-visible rounded-xl border bg-surface shadow-sm",
    ...
    >
      ...
      <div className={cn("flex items-center gap-2 rounded-t-xl bg-gradient-to-r px-3 py-2 to-transparent", meta.tone)}>
              ...
              options.map((opt) => (
                <div key={`${id}-opt-${opt.number}`} className="relative flex items-center justify-between text-[11px]">
                  ...
                  <Handle
                    ...
                    id={`a${opt.number}`}
                    className="!right-[-8px] !h-2.5 !w-2.5 !border-line !bg-white"
                  />
                </div>
              ))
            ) : (
              <p className="text-[11px] text-muted">Selecione os setores no painel</p>
            )}
          </div>
          <div className="relative flex items-center justify-between rounded-b-xl border-t border-line px-3 py-2">
            <span className="text-[11px] font-medium text-ink">Continuar</span>
            <Handle
              type="source"
              position={Position.Right}
              id="out"
...
export const PALETTE: { type: FlowNodeType; label: string; hint: string }[] = [
  ...
  { type: "menu", label: "Menu", hint: "Setores do WhatsApp" },

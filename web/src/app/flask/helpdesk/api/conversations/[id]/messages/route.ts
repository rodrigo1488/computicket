...
const MEDIA_TIMEOUT_MS = 270_000;
const TEXT_TIMEOUT_MS = 30_000;

const MEDIA_FAIL =
  "Não foi possível enviar o arquivo. Tente novamente; se persistir, envie um vídeo menor ou em MP4.";
const MEDIA_TIMEOUT = "O envio do arquivo demorou demais. Tente um vídeo menor ou em MP4.";
...
async function proxy(
  ...
  const target = `${FLASK.replace(/\/$/, "")}/helpdesk/api/conversations/${id}/messages${incoming.search}`;
  ...
  try {
    ...
    return NextResponse.json(
      { error: multipart ? (timedOut ? MEDIA_TIMEOUT : MEDIA_FAIL) : "Não foi possível falar com o Help Desk." },

...
class TechnicianLocation(db.Model):
    ...
    def to_dict(self):
        ...
        # Buscar usuário diretamente para garantir que temos os dados
    ...
    def get_all_technicians_with_last_location():
        """Retorna apenas usuários que têm localizações registradas"""
        from sqlalchemy import func
        from app.models import User
        
        # Subquery para obter a última localização de cada usuário
        subquery = db.session.query(
            TechnicianLocation.user_id,
            func.max(TechnicianLocation.last_seen).label('max_last_seen')
        ).group_by(TechnicianLocation.user_id).subquery()
        
        # Query principal para obter as últimas localizações
        last_locations = db.session.query(TechnicianLocation).join(
            subquery,
            (TechnicianLocation.user_id == subquery.c.user_id) &
            (TechnicianLocation.last_seen == subquery.c.max_last_seen)
        ).all()
        
        # Retornar apenas localizações que existem (não criar registros vazios)
